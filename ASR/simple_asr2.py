# HF code to use Whisper with silence filtering and without chunk prefixes in the output
...
def transcribe_audio(
    ...
    def transcribe_batch(batch, asr_pipeline_local, language):
        ...
        for i, a in enumerate(audio_list):
            ...
            if np.max(np.abs(a)) < silence_threshold:
                # Mark as silent by skipping ASR processing
                pass
            else:
        ...
        for i, a in enumerate(audio_list):
            if np.max(np.abs(a)) < silence_threshold:
                transcriptions[i] = ""  # or you could use "[silence]" if desired
            else:
                r = results[j]
    ...
    # Only write non-empty transcriptions, and omit the "[Chunk start...]" prefix.
    lines = []
